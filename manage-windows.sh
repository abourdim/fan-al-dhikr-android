#!/usr/bin/env bash
# ============================================================================
#  Fan al-Dhikr Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/21-fan-al-dhikr}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.fanaldhikr}"
AVD_NAME="${AVD_NAME:-fan_al_dhikr_test}"
...
cmd_test() {
  ...
  local out="/tmp/fan-al-dhikr-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Fan al-Dhikr Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
